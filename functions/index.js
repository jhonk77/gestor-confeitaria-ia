...
process.env.GCLOUD_PROJECT = process.env.GCLOUD_PROJECT || 'gestor-confeitaria-ia';
...
// CORREÇÃO APLICADA AQUI:
const vertexAI = new VertexAI({ project: 'gestor-confeitaria-ia', location: 'us-central1' });
const sheets = google.sheets({ version: 'v4', auth });
const drive = google.drive({ version: 'v3', auth });

// --- PONTO DE ENTRADA PRINCIPAL (HANDLER DA FUNÇÃO) ---
exports.assistenteHttp = functions.https.onCall(async (data, context) => {
    const { intent, payload, spreadsheetIds } = data;
    let responsePayload = {};

    console.log(`Recebida intenção: ${intent} com payload:`, payload);
