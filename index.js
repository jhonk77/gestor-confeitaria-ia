 ...
 * Ponto de entrada para todas as requisições do frontend.
 */
...
exports.assistenteHttp = functions.region('southamerica-east1').https.onCall(async (data, context) => {
    ...
    console.log(`Recebida intenção: ${intent}`);
    ...
    try {
        ...
        switch (intent) {
            case 'healthCheck':
                ...
                break;
            case 'setupDatabase':
                responsePayload = await agenteSetup.configurarPlanilhas(payload);
                break;
            case 'registrarDespesa':
                responsePayload = await agenteFinanceiro.registrarDespesa(payload);
                break;
            case 'processarDocumento':
                responsePayload = await agenteDocumentos.analisarNotaFiscal(payload);
                break;
            case 'registrarProducao':
                responsePayload = await agenteProducao.registrar(payload);
                break;
            case 'criarNovaReceita':
                responsePayload = await agenteInclusao.criarReceita(payload);
                break;
            case 'registrarPedido':
                responsePayload = await agenteCRM.registrarPedido(payload);
                break;
            case 'gerarAnalise':
                responsePayload = await agenteAnalista.gerarAnalise(payload);
                break;
            default:
...
async function getProximaLinhaVazia(spreadsheetId, range) {
...
const agenteSetup = {
    configurarPlanilhas: async ({ financeiroFile, operacoesFile }) => {
        const folderResponse = await drive.files.create({
            resource: { name: 'Base de Dados App Confeitaria', mimeType: 'application/vnd.google-apps.folder' },
            fields: 'id',
        });
        const folderId = folderResponse.data.id;

        const uploadAndConvert = async (fileData, fileName) => {
...
const agenteFinanceiro = {
    registrarDespesa: async ({ spreadsheetId, data, tipo, valor, fornecedor }) => {
...
const agenteCRM = {
    registrarPedido: async ({ spreadsheetId, cliente, produtos, dataEntrega, valor, status }) => {
...
const agenteInclusao = {
    criarReceita: async ({ spreadsheetId, recipeName, ingredients }) => {
        ...
        const rows = ingredients.map(ing => [ing.name, ing.quantity, ing.unit]);
...
const agenteProducao = {
    registrar: async ({ spreadsheetId, productName, quantity }) => {
...
const agenteEstoque = {
    darBaixa: async ({ spreadsheetId, item, quantidade }) => {
...
const agenteAnalista = {
    gerarAnalise: async ({ spreadsheetId, query }) => {
...
// --- EXPORTAÇÃO DOS AGENTES PARA USO NO HANDLER ---
exports.agenteSetup = agenteSetup;
exports.agenteFinanceiro = agenteFinanceiro;
exports.agenteDocumentos = agenteDocumentos;
exports.agenteCRM = agenteCRM;
exports.agenteInclusao = agenteInclusao;
exports.agenteProducao = agenteProducao;
exports.agenteEstoque = agenteEstoque;
exports.agenteAnalista = agenteAnalista;
// --- FIM DOS AGENTES ESPECIALISTAS ---   